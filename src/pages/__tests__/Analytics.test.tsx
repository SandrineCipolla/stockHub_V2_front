...
describe('Analytics Component', () => {
  ...
  describe('Initial render', () => {
    describe('when Analytics loads successfully', () => {
      ...
      it('should display Analytics title and description', async () => {
        ...
        expect(
          screen.getByText(/Prédictions Machine Learning basées sur régression linéaire/)
        ).toBeInTheDocument();
      });
    ...
    describe('when Analytics has ML predictions', () => {
      ...
      it('should display ML info box', async () => {
        ...
        expect(
          screen.getByText(/Régression linéaire avec méthode des moindres carrés/)
        ).toBeInTheDocument();
      });
    });
  });
  ...
  describe('Design System integration', () => {
    describe('when using web components', () => {
      it('should render sh-stat-card components correctly', async () => {
        // Analytics uses StatCard wrapper which renders sh-stat-card
        // TODO: Add dedicated StatCard wrapper tests in separate PR
        vi.spyOn(useStocksModule, 'useStocks').mockReturnValue(
          createMockUseStocks({
            stocks: dashboardStocks.slice(0, 5),
          })
        ...
        // Check for sh-stat-card web components
        const statCards = container.querySelectorAll('sh-stat-card');
        expect(statCards.length).toBeGreaterThan(0);

        // Verify all stat cards have theme
    ...
    describe('when using deprecated sh-card test', () => {
      it.skip('DEPRECATED: Old test for sh-card variants - Analytics now uses sh-stat-card', async () => {
        // This test is obsolete - Analytics migrated from sh-card to sh-stat-card
        vi.spyOn(useStocksModule, 'useStocks').mockReturnValue(
          createMockUseStocks({
            stocks: dashboardStocks.slice(0, 5),
          })
        ...
        // Old code checking for sh-card (no longer used)
