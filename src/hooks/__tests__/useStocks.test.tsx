    ...
    describe('when hook is initialized', () => {
      ...
      it('should handle fixture stock data correctly', async () => {
        ...
        await waitFor(() => {
          ...
          const hasOptimalStock = result.current.stocks.some(stock => stock.status === 'optimal');
  ...
  describe('filtering and stats', () => {
      ...
      it('should handle all fixture status types', async () => {
        ...
        for (const status of stockStatuses) {
          ...
          await waitFor(() => {
            ...
            expect(result.current.stats?.[status as keyof typeof result.current.stats]).toBe(
              statusStocks.length
            );
          });
        }
    ...
    describe('when filtering by category', () => {
      it('should filter stocks by category correctly', async () => {
        ...
        await waitFor(() => {
          const electronicsStocks = result.current.stocks.filter(
            s => s.category === stockCategories.electronics
          );
