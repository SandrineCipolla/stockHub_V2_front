...
export const StatusBadge: React.FC<StatusBadgeProps> = ({
  ...
  const colorClasses = theme === 'dark' ? config.colors.dark.badge : config.colors.light.badge;
  ...
  return (
    ...
    >
      {showIcon && <Icon className={STATUS_BADGE_ICON_SIZE_CLASSES[size]} aria-hidden="true" />}
      <span className={STATUS_BADGE_LABEL_CLASS}>{config.label}</span>
    </span>
  );
};
