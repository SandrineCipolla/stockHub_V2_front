...
| Le besoin d'un refresh token automatique n'apparaîtra pas à court terme                                                    | Hypothèse | Non vérifié — dépend de la façon dont Azure AD B2C (MSAL) gère déjà le renouvellement de token côté frontend ; à confirmer en lisant `src/config/authConfig.ts` |
...
- Zéro dépendance supplémentaire à auditer et à maintenir — pertinent pour un projet solo.
...
- **Inconvénients :** dépendance supplémentaire (~15KB gzippé) pour des besoins non encore observés ; l'app cible des navigateurs modernes où `fetch` couvre déjà le besoin
- **Pourquoi rejetée :** aucune capacité manquante aujourd'hui ne justifie le coût de dépendance — réévaluable si le besoin d'intercepteur apparaît (voir Réexamen)
...
- **Pourquoi rejetée :** même raisonnement qu'Axios, en plus léger — pas encore de preuve que `fetch` brut pose un problème concret
...
| Mesure            | **hypothèse à vérifier** : aujourd'hui chaque call site gère son erreur individuellement (pas de gestion centralisée) — pas encore mesuré si cela produit des comportements incohérents entre écrans |
